...
@section('content')
            ...
            <form action="{{ route('superadmin.kelola-undangan.update', $invitation->id) }}" method="POST" class="space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Judul Undangan</label>
                            <input type="text" name="title" id="title" value="{{ old('title', $invitation->title) }}" required oninput="generateSlug()"
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Slug URL</label>
                            <input type="text" name="slug" id="slug" value="{{ old('slug', $invitation->slug) }}" required
                                class="w-full border border-slate-200 focus:border-sky-500 rounded-xl px-4 py-2.5 text-sm focus:outline-hidden transition-all">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Tipe Undangan</label>
                            <select name="invitation_type_id" id="invitation_type_id" required onchange="toggleProfileFields()"
                                ...
                                @foreach ($types as $type)
                                    <option value="{{ $type->id }}" data-slug="{{ Str::slug($type->name) }}"
                                        {{ old('invitation_type_id', $invitation->invitation_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('invitation_type_id') <p class="text-rose-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Tema Undangan</label>
                            <select name="theme_id" required
                                class="w-full border border-slate-200 focus:border-sky-500 rounded-xl px-4 py-2.5 text-sm bg-white focus:outline-hidden transition-all">
                                <option value="">-- Pilih Tema --</option>
                                @foreach ($themes as $theme)
                                    <option value="{{ $theme->id }}" {{ old('theme_id', $invitation->theme_id) == $theme->id ? 'selected' : '' }}>{{ $theme->name }}</option>
                                @endforeach
                            </select>
                            @error('theme_id') <p class="text-rose-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4 pt-4 border-t border-slate-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label id="label-first-name" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Nama Pihak Pertama / Pemilik</label>
                            <input type="text" name="first_name" value="{{ old('first_name', $invitation->profile->first_name ?? '') }}" required
                                class="w-full border border-slate-200 focus:border-sky-500 rounded-xl px-4 py-2.5 text-sm focus:outline-hidden transition-all">
                        </div>
                        <div>
                            <label id="label-first-nick" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Nama Panggilan Pihak Pertama</label>
                            <input type="text" name="first_nickname" value="{{ old('first_nickname', $invitation->profile->first_nickname ?? '') }}" required
                                class="w-full border border-slate-200 focus:border-sky-500 rounded-xl px-4 py-2.5 text-sm focus:outline-hidden transition-all">
                        </div>
                    </div>
                    ...
                    <div id="wrapper-second-party" class="grid grid-cols-1 md:grid-cols-2 gap-4 hidden">
                        <div>
                            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Nama Pihak Kedua</label>
                            <input type="text" name="second_name" id="second_name" value="{{ old('second_name', $invitation->profile->second_name ?? '') }}"
                                class="w-full border border-slate-200 focus:border-sky-500 rounded-xl px-4 py-2.5 text-sm focus:outline-hidden transition-all">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Nama Panggilan Pihak Kedua</label>
                            <input type="text" name="second_nickname" id="second_nickname" value="{{ old('second_nickname', $invitation->profile->second_nickname ?? '') }}"
                                class="w-full border border-slate-200 focus:border-sky-500 rounded-xl px-4 py-2.5 text-sm focus:outline-hidden transition-all">
                        </div>
                    </div>
                    ...
                    <div id="wrapper-parents" class="grid grid-cols-1 md:grid-cols-2 gap-4 hidden">
                        <div class="p-4 bg-slate-50/50 rounded-xl border border-slate-100 space-y-3">
                            <span class="text-[11px] font-bold text-slate-400 uppercase block tracking-wider">Orang Tua Pihak ke-1</span>
                            <div>
                                <label class="block text-2xs font-semibold text-slate-500 uppercase mb-1">Nama Ayah</label>
                                <input type="text" name="first_father" id="first_father" value="{{ old('first_father', $invitation->profile->first_father ?? '') }}"
                            ...
                            <div>
                                ...
                                <input type="text" name="first_mother" id="first_mother" value="{{ old('first_mother', $invitation->profile->first_mother ?? '') }}"
                                    class="w-full border border-slate-200 focus:border-sky-500 rounded-lg px-3 py-2 text-xs bg-white focus:outline-hidden transition-all">
                            </div>
                        </div>
                        
                        <div class="p-4 bg-slate-50/50 rounded-xl border border-slate-100 space-y-3">
                            <span class="text-[11px] font-bold text-slate-400 uppercase block tracking-wider">Orang Tua Pihak ke-2</span>
                            <div>
                                <label class="block text-2xs font-semibold text-slate-500 uppercase mb-1">Nama Ayah</label>
                                <input type="text" name="second_father" id="second_father" value="{{ old('second_father', $invitation->profile->second_father ?? '') }}"
                                    class="w-full border border-slate-200 focus:border-sky-500 rounded-lg px-3 py-2 text-xs bg-white focus:outline-hidden transition-all">
                            </div>
                            <div>
                                <label class="block text-2xs font-semibold text-slate-500 uppercase mb-1">Nama Ibu</label>
                                <input type="text" name="second_mother" id="second_mother" value="{{ old('second_mother', $invitation->profile->second_mother ?? '') }}"
                                    class="w-full border border-slate-200 focus:border-sky-500 rounded-lg px-3 py-2 text-xs bg-white focus:outline-hidden transition-all">
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Alamat Utama Pelaksanaan Acara</label>
                        <textarea name="address" rows="2" class="w-full border border-slate-200 focus:border-sky-500 rounded-xl px-4 py-2.5 text-sm focus:outline-hidden transition-all">{{ old('address', $invitation->profile->address ?? '') }}</textarea>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-2 pt-4 border-t border-slate-50">
                    ...
                    <button type="submit"
                        class="px-4 py-2.5 text-sm font-medium text-white bg-sky-500 hover:bg-sky-600 rounded-xl shadow-sm transition-all">Perbarui Undangan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function toggleProfileFields() {
            ...
            const isCoupleEvent = typeSlug.includes('pernikahan') || typeSlug.includes('wedding') || typeSlug.includes('tunangan') || typeSlug.includes('engagement');
            ...
            if (isWeddingEvent) {
                ...
                fFather.required = true; fMother.required = true;
                sFather.required = true; sMother.required = true;
            } else {
                parentsWrapper.classList.add('hidden');
                fFather.required = false; fMother.required = false;
                sFather.required = false; sMother.required = false;
            }
        }
