...
<head>
    ...
    @php
        ...
        $coverImage = ($coverMedia && file_exists(public_path($coverMedia->file_path)))
            ...
            : 'https://images.unsplash.com/photo-1530103862676-de8892bf30b5?q=80&w=2000';
...
<body class="antialiased selection:bg-[#D4AF37] selection:text-black envelope-active">
    ...
    <div id="envelopeOverlay" class="fixed inset-0 z-50 flex items-center justify-center bg-[#111111] transition-all duration-1000 transform translate-y-0">
        ...
        <div class="max-w-xl text-center px-6 z-10">
            ...
            <h2 class="heading-font text-3xl text-white mb-8">
                The Milestone Of <br>
                <span class="text-5xl md:text-6xl gold-text italic" data-preview="first_name">{{ $firstName }}</span>
            </h2>
    ...
    <section class="py-24 px-6 bg-[#111111] text-center border-t border-[#222]">
        <div class="max-w-3xl mx-auto">
            <div class="w-48 h-48 md:w-56 md:h-56 mx-auto overflow-hidden rounded-full shadow-lg border-4 gold-border mb-6">
                <img src="{{ $firstPhotoPath }}" class="w-full h-full object-cover grayscale hover:grayscale-0 transition duration-500">
            </div>
            <h3 class="heading-font text-4xl gold-text mb-4" data-preview="first_name">{{ $firstName }}</h3>
            @if($showParents)
            ...
            @forelse ($invitation->galleries as $index => $gallery)
                <div class="overflow-hidden">
                    @if(file_exists(public_path($gallery->file_path)))
                        <img src="{{ asset($gallery->file_path) }}" class="w-full h-64 object-cover filter grayscale hover:grayscale-0 transition duration-700">
                    @else
                        <img src="https://images.unsplash.com/photo-1530103862676-de8892bf30b5?q=80&w=800" class="w-full h-64 object-cover filter grayscale hover:grayscale-0 transition duration-700">
                    @endif
                </div>
            @empty
                @for ($i = 0; $i < 4; $i++)
                    <div class="overflow-hidden">
                        <img src="https://images.unsplash.com/photo-1530103862676-de8892bf30b5?q=80&w=800" class="w-full h-64 object-cover filter grayscale">
                    </div>
                @endfor
            @endforelse
        </div>
    ...
    <section class="py-24 px-6 bg-[#0A0A0A] text-center border-t border-[#222]">
        <div class="max-w-2xl mx-auto">
            ...
            <h3 class="heading-font text-3xl gold-text" data-preview="first_name">{{ $firstName }}</h3>
        </div>
    </section>
    @endif
