...
.cv-name{
    ...
    color: white; line-height: 1.05; margin-bottom: 0.25rem;
}
.cv-sub {
...
.cv-from strong { color: rgba(255,255,255,0.7); }
.cv-btn{
    ...
    box-shadow: 0 8px 24px rgba(255,53,96,0.4);
    transition: transform 0.2s, box-shadow 0.2s;
}
.cv-btn:hover  { transform: translateY(-3px); box-shadow: 0 14px 32px rgba(255,53,96,0.5); }
...
.h-body {
    ...
    padding: 0 1.5rem; width: 100%; gap: 0;
}
...
.h-eline{
    ...
    background: var(--amber); opacity: 0.5; border-radius: 1px;
}

.h-clip { overflow: hidden; line-height: 0.88; margin-bottom: 0.5rem; }
...
.h-strip{
    ...
    animation: fadeUp 0.5s 2.7s both;
    flex-wrap: wrap; justify-content: center;
}
.hs-cell { padding: 0.85rem 1.4rem; text-align: center; }
.hs-cell .l{
...
.hs-cell .v{
    font-family: 'Nunito', sans-serif;
    font-weight: 800; font-size: 0.9rem; color: white; line-height: 1.25;
}
.hs-sep {
...
.h-hint{
    ...
    color: rgba(255,255,255,0.18); font-size: 0.58rem;
...
.ab-glow{
    position: absolute; border-radius: 50%;
    filter: blur(70px); pointer-events: none; opacity: 0.45;
}
.ab-inner{
...
.ab-name{
    ...
    color: var(--night); line-height: 1; margin-bottom: 0.55rem;
}
.ab-quote{
    font-size: 0.84rem; font-style: italic; color: #999;
    line-height: 1.75;
...
.pb-head{
    ...
    color: var(--violet); margin-bottom: 0.15rem;
}
.pb-row { display: flex; align-items: center; gap: 0.6rem; }
.pb-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.dot-dad { background: #60A5FA; }
.dot-mom { background: var(--coral); }
.pb-name { font-weight: 700; font-size: 0.9rem; color: var(--night); }
.pb-role { font-size: 0.62rem; color: #bbb; margin-left: auto; font-weight: 600; }
...
.ev-kicker{
    ...
    color: var(--violet); margin-bottom: 0.3rem;
}
.ev-title{
...
.ec-name{
    ...
    font-size: 1.15rem; color: white; line-height: 1.2;
}

.ec-perf{
    margin: 0 0.8rem; display: flex; align-items: center; position: relative;
}
.ec-perf::before, .ec-perf::after{
    content: '';
    position: absolute; width: 18px; height: 18px;
    border-radius: 50%; background: #F5F0FF;
...
.ec-dash{
    ...
    background: repeating-linear-gradient(
        90deg, rgba(124,58,237,0.18) 0,rgba(124,58,237,0.18) 6px,
...
.ec-row { display: flex; align-items: flex-start; gap: 0.7rem; }
.ec-ic{
...
.ec-row .v{ font-size: 0.84rem; font-weight: 700; color: var(--night); line-height: 1.3; }
...
.ec-maps{
    ...
    transition: opacity 0.2s; margin-top: 0.2rem;
}
...
.gal-top{
    ...
    flex-shrink: 0; position: relative; z-index: 2; width: 100%;
}
.gal-kicker{
    font-size: 0.62rem; font-weight: 800;
    letter-spacing: 3px; text-transform: uppercase;
...
.gal-card:hover { transform: rotate(0) scale(1.08) translateY(-10px) !important; z-index: 5; }
.gal-card img {
...
.rsvp-form { display: flex; flex-direction: column; gap: 0.7rem; }

.rf-group { display: flex; flex-direction: column; gap: 0.28rem; }
.rf-label{
...
.rf-input{
    ...
    font-size: 0.93rem; font-family: 'Nunito', sans-serif;
...
.rf-input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(255,53,96,0.08);
}

/* Radio pills */
.rf-pills { display: flex; gap: 0.65rem; }
...
.rf-pill-label input:checked + .rf-pill-btn{
    border-color: var(--coral);
    background: rgba(255,53,96,0.06);
    color: var(--coral);
    box-shadow: 0 0 0 3px rgba(255,53,96,0.08);
}
...
.rc-btn:hover { color: var(--coral); background: rgba(255,53,96,0.05); }
.rc-val{
...
.rf-submit{
    ...
    cursor: pointer;
    box-shadow: 0 6px 20px rgba(255,53,96,0.32);
    transition: transform 0.2s, box-shadow 0.2s;
    display: flex; align-items: center; justify-content: center;
    gap: 0.5rem; width: 100%; margin-top: 0.3rem;
}
.rf-submit:hover {
    transform: translateY(-2px);
    box-shadow: 0 10px 28px rgba(255,53,96,0.42);
}
...
.rsvp-form.hide     { display: none; }
.rsvp-header.hide   { display: none; }

.success-icon { font-size: 3.5rem; animation: successPop 0.6s cubic-bezier(0.34,1.56,0.64,1) both; }
.success-title{
    font-family: 'Nunito', sans-serif; font-weight: 900;
    font-size: 1.6rem; color: var(--night);
}
.success-text { font-size: 0.88rem; color: #999; line-height: 1.7; }
...
.cl-emoji { font-size: clamp(2rem,7vw,3.2rem); margin-bottom: 0.7rem; line-height: 1; }
.cl-title{
    font-family: 'Nunito', sans-serif; font-weight: 900;
    font-size: clamp(2.2rem,8vw,3.8rem);
    color: white; line-height: 1.05; margin-bottom: 0.8rem;
...
.cl-name{
    ...
    font-size: clamp(2rem,7vw,3rem); color: white; line-height: 1;
    text-shadow: 0 2px 12px rgba(0,0,0,0.1);
}
.cl-parents {
    font-size: 0.83rem; color: rgba(255,255,255,0.55); margin-top: 0.35rem;
}
...
.n-btn.active .n-pip{
    background: rgba(255,53,96,0.13);
...
.n-btn.active .n-ico { transform: scale(1.08); }

.n-lbl{
    font-size: 0.57rem; font-weight: 600;
    letter-spacing: 0.2px; color: #c4bfc8;
    transition: color 0.22s; white-space: nowrap;
}
.n-btn.active .n-lbl { color: var(--coral); font-weight: 800; }
...
#musicBtn{
    ...
    background: rgba(13,10,26,0.6); backdrop-filter: blur(12px);
    color: rgba(255,255,255,0.6); font-size: 0.88rem; cursor: pointer;
...
<div id="cover">
    ...
    <div class="cv-blob" style="width:360px;height:360px;background:rgba(255,53,96,0.2);top:-100px;right:-80px;"></div>
    <div class="cv-blob" style="width:280px;height:280px;background:rgba(124,58,237,0.18);bottom:-60px;left:-70px;"></div>
    <div class="cv-blob" style="width:180px;height:180px;background:rgba(245,158,11,0.15);bottom:25%;right:12%;"></div>
...
<div id="scroller">
    ...
    <section class="snap" id="s-hero" data-section="0">
        ...
        @endif
        <div class="hero-glow" style="width:420px;height:420px;background:rgba(255,53,96,0.14);top:-110px;left:50%;transform:translateX(-50%);z-index:1;"></div>
        <div class="hero-glow" style="width:300px;height:300px;background:rgba(124,58,237,0.12);bottom:20%;right:-80px;"></div>
        <div class="hero-glow" style="width:240px;height:240px;background:rgba(245,158,11,0.1);bottom:12%;left:-60px;"></div>
        ...
        <div class="h-hint">
            <i class="fa-solid fa-chevron-down" style="font-size:0.85rem;"></i>Scroll
        </div>
    </section>
    ...
    <section class="snap" id="s-about" data-section="1">
        <div class="ab-glow" style="width:350px;height:350px;background:rgba(255,53,96,0.07);top:-100px;right:-80px;"></div>
        <div class="ab-glow" style="width:300px;height:300px;background:rgba(124,58,237,0.07);bottom:-80px;left:-90px;"></div>
    ...
    <section class="snap" id="s-rsvp" data-section="4">
        ...
        {{-- RSVP bg blobs --}}
        <div style="position:absolute;border-radius:50%;filter:blur(70px);width:300px;height:300px;background:rgba(255,53,96,0.06);top:-80px;right:-80px;pointer-events:none;z-index:0;"></div>
        <div style="position:absolute;border-radius:50%;filter:blur:70px;width:250px;height:250px;background:rgba(124,58,237,0.06);bottom:-60px;left:-60px;pointer-events:none;z-index:0;"></div>
            ...
            <form class="rsvp-form" id="rsvpForm"
                ...
                <div class="rf-group">
                    ...
                    <textarea name="message" class="rf-textarea"
                              placeholder="Tulis ucapan atau pesan untuk {{ $invitation->profile->first_name }}…"></textarea>
                </div>
    ...
    <section class="snap" id="s-closing" data-section="5">
        ...
        <div class="cl-rings">
            ...
            <div class="cl-ring" style="width:200px;height:200px;bottom:-40px;right:-50px;"></div>
        </div>
        <div class="cl-body">
...
<nav id="bottom-nav">
    ...
    <button class="n-btn" data-target="s-rsvp"    onclick="navTo(this)">
        <div class="n-pip"><span class="n-ico">🎟️</span></div>
...
document.querySelectorAll('input[name="attendance"]').forEach(radio => {
    radio.addEventListener('change', () => {
        const grp = document.getElementById('guestCountGroup');
        if (grp) grp.style.display = radio.value === 'hadir' ? 'flex' : 'none';
    });
});
...
function submitRsvp(e) {
    ...
    /* Optional: send via fetch
    fetch(form.action, { method: 'POST', body: fd })
        .then(() => showRsvpSuccess())
        .catch(() => showRsvpSuccess()); // show success anyway for demo
    */

    showRsvpSuccess(); // demo — remove & uncomment fetch for production
}
...
function burstConfetti() {
    ...
    const set  = () => { canvas.width = innerWidth; canvas.height = innerHeight; };
    ...
    (function draw(){
        ...
        bits.forEach(p=>{
            ...
            ctx.globalAlpha=0.9;ctx.fillStyle=p.c;
