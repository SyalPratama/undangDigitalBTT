...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Fredoka:wght@400;500;600;700&family=Nunito:ital,wght@0,400;0,500;0,600;0,700;0,800;0,900;1,500&display=swap" rel="stylesheet">
...
.cloud::before { width: 55%; height: 200%; top: -60%; left: 18%; }
.cloud::after  { width: 40%; height: 160%; top: -45%; right: 15%; }

/* ── Grass strip (bottom) ── */
.grass-strip {
    position: absolute; bottom: var(--nav-pb); left: 0; right: 0;
    height: 40px; pointer-events: none; overflow: hidden;
}
.grass-strip svg { display: block; width: 100%; height: 100%; }
...
.cv-dot { position: absolute; border-radius: 50%; animation: twinkle ease-in-out infinite alternate; }
...
.cv-moon  { position: absolute; top: 8%; right: 10%; font-size: clamp(2.5rem,8vw,4.5rem); animation: moonGlow 4s ease-in-out infinite; }

.cv-body  {
    position: relative; z-index: 2; text-align: center;
    padding: 2rem 1.5rem; max-width: 440px; width: 100%;
}
.cv-eyebrow { font-size: .7rem; font-weight: 700; letter-spacing: 3.5px; text-transform: uppercase; color: rgba(253,230,138,.6); margin-bottom: .5rem; }
.cv-label   {
    font-family: 'Fredoka', sans-serif; font-weight: 700;
    font-size: clamp(1rem, 3.5vw, 1.3rem);
    color: var(--gold); letter-spacing: 2px; margin-bottom: .4rem;
}
.cv-name    {
    font-family: 'Fredoka', sans-serif; font-weight: 700;
    font-size: clamp(2.5rem, 10vw, 4rem); color: white; line-height: 1.05; margin-bottom: .25rem;
}
.cv-sub     { font-size: .85rem; color: rgba(255,255,255,.38); margin-bottom: 1.6rem; }
.cv-guest   {
    background: rgba(134,239,172,.1); border: 1.5px solid rgba(134,239,172,.25);
    border-radius: 14px; padding: .8rem 2rem;
    color: #86EFAC; font-size: 1rem; font-weight: 700;
...
.cv-btn    {
    ...
    cursor: pointer; box-shadow: 0 8px 24px rgba(22,163,74,.4);
    transition: transform .2s, box-shadow .2s;
}
.cv-btn:hover  { transform: translateY(-3px); box-shadow: 0 14px 32px rgba(22,163,74,.5); }
...
.h-bismillah {
    font-size: .7rem; font-weight: 800; letter-spacing: 3.5px; text-transform: uppercase;
    color: rgba(255,255,255,.55); margin-bottom: .7rem;
    display: flex; align-items: center; gap: .6rem;
    animation: fadeUp .5s 2s both;
}
.h-bis-line { display: inline-block; width: 20px; height: 1px; background: rgba(255,255,255,.4); }

.h-aqiqah {
    font-family: 'Fredoka', sans-serif; font-weight: 600;
    font-size: clamp(.9rem,2.5vw,1.1rem);
    color: #FDE68A; letter-spacing: 3px; text-transform: uppercase;
    margin-bottom: .5rem; animation: fadeUp .5s 2.1s both;
}
.h-clip { overflow: hidden; line-height: .9; margin-bottom: .5rem; }
.h-name {
...
.h-tag { font-size: .88rem; color: rgba(255,255,255,.5); letter-spacing: .03em; margin-bottom: 1.4rem; animation: fadeUp .5s 2.6s both; }
...
.hs-cell .l { font-size: .54rem; font-weight: 800; letter-spacing: 2px; text-transform: uppercase; color: rgba(255,255,255,.45); margin-bottom: .22rem; }
.hs-cell .v { font-family: 'Nunito', sans-serif; font-weight: 800; font-size: .88rem; color: white; line-height: 1.25; }
.hs-sep     { width: 1px; height: 34px; background: rgba(255,255,255,.2); flex-shrink: 0; }

.h-hint {
    position: absolute; bottom: calc(var(--nav-pb) + .4rem); left: 50%; transform: translateX(-50%);
    z-index: 3; display: flex; flex-direction: column; align-items: center; gap: .2rem;
    color: rgba(255,255,255,.3); font-size: .55rem; font-weight: 800; letter-spacing: 2.5px; text-transform: uppercase;
...
.cloud-frame {
    position: relative; width: min(170px, 42vw); aspect-ratio: 1;
...
.cloud-frame::before {
    ...
    animation: cloudFrameRot 10s linear infinite; z-index: 0;
}
.cloud-frame img, .cloud-frame .cf-ph {
...
.ab-kicker { font-size: .62rem; font-weight: 800; letter-spacing: 3.5px; text-transform: uppercase; color: var(--grass); margin-bottom: .3rem; }
.ab-name   { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: clamp(2.2rem,8vw,3.5rem); color: var(--sage); line-height: 1; margin-bottom: .55rem; }
.ab-quote  { font-size: .84rem; font-style: italic; color: #6b7280; line-height: 1.75; border-left: 2.5px solid var(--grass); padding-left: .85rem; margin-bottom: .9rem; text-align: left; }

.parents-block { background: white; border-radius: 14px; padding: .85rem 1.1rem; display: flex; flex-direction: column; gap: .5rem; box-shadow: 0 2px 12px rgba(22,163,74,.1); }
.pb-head { font-size: .58rem; font-weight: 800; letter-spacing: 2.5px; text-transform: uppercase; color: var(--grass); margin-bottom: .15rem; }
...
.ev-kicker  { font-size: .62rem; font-weight: 800; letter-spacing: 3px; text-transform: uppercase; color: var(--grass); margin-bottom: .3rem; }
.ev-title   { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: clamp(1.7rem,6vw,2.6rem); color: var(--sage); line-height: 1.1; }
...
.ev-card:hover { transform: translateY(-5px); box-shadow: 0 14px 40px rgba(22,163,74,.18); }
...
.ec-name  { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: 1.2rem; color: white; line-height: 1.2; }
...
.ec-dash { flex: 1; height: 1.5px; background: repeating-linear-gradient(90deg, rgba(22,163,74,.2) 0, rgba(22,163,74,.2) 6px, transparent 6px, transparent 12px); }
...
.ec-maps {
    ...
    background: var(--grass); color: white; text-decoration: none;
    padding: .5rem 1.1rem; border-radius: 50px;
    font-size: .74rem; font-weight: 700; transition: opacity .2s; margin-top: .2rem;
}
...
.gal-kicker { font-size: .62rem; font-weight: 800; letter-spacing: 3px; text-transform: uppercase; color: #86EFAC; margin-bottom: .25rem; }
.gal-title  { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: clamp(1.5rem,5vw,2.3rem); color: white; }
...
.gal-card {
    flex-shrink: 0; scroll-snap-align: center; background: white;
...
.rsvp-inner {
    width: 100%; max-width: 420px; display: flex; flex-direction: column; gap: .9rem;
    position: relative; z-index: 1; padding-bottom: var(--nav-pb);
}
.rsvp-header { text-align: center; }
.rsvp-kicker { font-size: .62rem; font-weight: 800; letter-spacing: 3px; text-transform: uppercase; color: var(--grass); margin-bottom: .3rem; }
.rsvp-title  { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: clamp(1.6rem,6vw,2.4rem); color: var(--night); line-height: 1.1; }
...
.rf-input:focus { border-color: var(--grass); box-shadow: 0 0 0 3px rgba(22,163,74,.08); }
...
.rf-pill-label input:checked + .rf-pill-btn {
    border-color: var(--grass); background: rgba(22,163,74,.06); color: var(--grass);
    box-shadow: 0 0 0 3px rgba(22,163,74,.08);
}
.rf-counter { display: flex; align-items: center; border: 2px solid #D1FAE5; border-radius: 12px; background: #F0FDF4; overflow: hidden; }
.rc-btn { width: 44px; height: 44px; border: none; background: none; font-size: 1.3rem; font-weight: 700; cursor: pointer; color: #bbb; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: color .2s, background .2s; }
.rc-btn:hover { color: var(--grass); background: rgba(22,163,74,.05); }
.rc-val { flex: 1; text-align: center; font-family: 'Nunito', sans-serif; font-weight: 800; font-size: 1.1rem; color: var(--night); }
.rf-textarea { border: 2px solid #D1FAE5; border-radius: 12px; padding: .75rem 1rem; font-size: .9rem; font-family: 'Nunito', sans-serif; font-weight: 500; color: var(--night); background: #F0FDF4; outline: none; resize: none; height: 70px; width: 100%; transition: border-color .2s, box-shadow .2s; }
.rf-textarea:focus { border-color: var(--grass); box-shadow: 0 0 0 3px rgba(22,163,74,.08); }
.rf-submit {
    background: linear-gradient(135deg, #16A34A, #14532D);
    color: white; border: none; border-radius: 50px; padding: .9rem;
    font-family: 'Nunito', sans-serif; font-weight: 800; font-size: 1rem;
    cursor: pointer; box-shadow: 0 6px 20px rgba(22,163,74,.32);
    transition: transform .2s, box-shadow .2s;
    display: flex; align-items: center; justify-content: center; gap: .5rem; width: 100%; margin-top: .3rem;
}
.rf-submit:hover { transform: translateY(-2px); box-shadow: 0 10px 28px rgba(22,163,74,.42); }
...
.success-title { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: 1.7rem; color: var(--night); }
...
.cl-body {
    ...
    text-align: center; max-width: 460px; width: 100%; padding: 0 1.5rem var(--nav-pb);
}
.cl-dua    { font-size: .72rem; font-weight: 700; letter-spacing: 2.5px; text-transform: uppercase; color: rgba(253,230,138,.7); margin-bottom: .6rem; }
.cl-emoji  { font-size: clamp(2rem,7vw,3.2rem); margin-bottom: .7rem; line-height: 1.3; }
.cl-title  { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: clamp(2.2rem,8vw,3.8rem); color: white; line-height: 1.05; margin-bottom: .8rem; text-shadow: 0 2px 20px rgba(0,0,0,.12); }
.cl-text   { font-size: .9rem; color: rgba(255,255,255,.82); line-height: 1.85; margin-bottom: 1.5rem; max-width: 340px; }
.cl-divider{ width: 40px; height: 1.5px; background: rgba(253,230,138,.5); border-radius: 1px; margin-bottom: 1.5rem; }
...
.cl-name   { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: clamp(2rem,7vw,3rem); color: #FDE68A; line-height: 1; text-shadow: 0 2px 12px rgba(0,0,0,.1); }
...
.n-btn.active { background: var(--grass); flex: 1.75; }
.n-btn:active { opacity: .8; }
.n-ico { font-size: 1.2rem; line-height: 1; flex-shrink: 0; transition: transform .3s cubic-bezier(.34,1.56,.64,1); }
...
#musicBtn {
    ...
    background: rgba(12,26,46,.65); backdrop-filter: blur(12px);
    color: rgba(255,255,255,.65); font-size: .88rem; cursor: pointer;
...
@keyframes moonGlow {
    0%,100% { filter: drop-shadow(0 0 8px rgba(253,230,138,.4)); transform: scale(1); }
    50%      { filter: drop-shadow(0 0 20px rgba(253,230,138,.7)); transform: scale(1.08); }
}
@keyframes nameUp {
...
@keyframes cloudFrameRot { to { transform: rotate(360deg); } }
@keyframes gradFlow {
...
<div id="cover">
    ...
    <div class="blob" style="width:320px;height:320px;background:rgba(22,163,74,.12);top:-70px;right:-60px;"></div>
    <div class="blob" style="width:260px;height:260px;background:rgba(56,189,248,.1);bottom:-50px;left:-60px;"></div>
    <div class="blob" style="width:180px;height:180px;background:rgba(234,179,8,.08);bottom:20%;right:15%;"></div>

    {{-- Moon & stars --}}
    <span class="cv-moon">🌙</span>
    <span class="deco" style="top:10%;left:10%;font-size:1.5rem;animation:twinkle 2s 0s ease-in-out infinite alternate;">⭐</span>
    <span class="deco" style="top:15%;right:20%;font-size:1rem;animation:twinkle 1.8s .5s ease-in-out infinite alternate;">✨</span>
    <span class="deco" style="top:35%;left:5%;font-size:.9rem;animation:twinkle 2.5s 1s ease-in-out infinite alternate;">⭐</span>
    <span class="deco" style="top:25%;right:5%;font-size:1.2rem;animation:twinkle 2.2s .3s ease-in-out infinite alternate;">🌟</span>

    {{-- Sleeping sheep --}}
    <span class="cv-sheep" style="left:8%;">🐑</span>
    <span class="cv-sheep" style="right:6%;animation-duration:4s;animation-delay:.5s;font-size:clamp(2.5rem,8vw,4.5rem);">🐑</span>
...
<div id="scroller">
    ...
    <section class="snap" id="s-hero" data-section="0">
        @if ($invitation->cover?->file_path)
            <div style="position:absolute;inset:0;background-image:url('{{ asset('storage/'.$invitation->cover->file_path) }}');background-size:cover;background-position:center;opacity:.06;z-index:0;"></div>
        @endif

        {{-- Clouds --}}
        <div class="cloud" style="width:180px;height:45px;top:14%;left:-30px;animation-duration:28s;animation-delay:0s;opacity:.6;"></div>
        <div class="cloud" style="width:120px;height:32px;top:22%;right:-20px;animation-duration:22s;animation-delay:-10s;opacity:.5;"></div>
        <div class="cloud" style="width:200px;height:50px;top:8%;right:10%;animation-duration:35s;animation-delay:-5s;opacity:.4;"></div>
        <div class="cloud" style="width:90px;height:24px;top:30%;left:20%;animation-duration:20s;animation-delay:-15s;opacity:.45;"></div>

        {{-- Floating sheep --}}
        <span class="deco" style="bottom:calc(var(--nav-pb) + 30px);left:4%;font-size:clamp(2rem,6vw,3.5rem);z-index:2;animation:sheepBounce 3s ease-in-out infinite;">🐑</span>
        <span class="deco" style="bottom:calc(var(--nav-pb) + 20px);right:5%;font-size:clamp(1.5rem,4vw,2.5rem);z-index:2;animation:sheepBounce 3.5s .5s ease-in-out infinite;">🐑</span>
        <span class="deco" style="top:10%;left:6%;font-size:1.2rem;z-index:2;animation:twinkle 2s ease-in-out infinite alternate;color:#FDE68A;">⭐</span>
        <span class="deco" style="top:8%;right:7%;font-size:1.5rem;z-index:2;animation:twinkle 2.5s .5s ease-in-out infinite alternate;color:#FDE68A;">🌟</span>

        <div id="starField" style="position:absolute;inset:0;pointer-events:none;z-index:1;"></div>

        <div class="h-body">
            <div class="h-bismillah"><span class="h-bis-line"></span>Bismillahirrahmanirrahim<span class="h-bis-line"></span></div>
            <p class="h-aqiqah">🌿 Aqiqah 🌿</p>
            <div class="h-clip">
                <span class="h-name">{{ $invitation->profile->first_name }}</span>
            </div>
            <p class="h-tag">🤲 Dengan penuh syukur atas nikmat Allah SWT, kami mengundang kehadiran Bapak/Ibu/Saudara/i</p>
    ...
    <section class="snap" id="s-about" data-section="1">
        <div class="blob" style="width:300px;height:300px;background:rgba(22,163,74,.09);top:-80px;right:-70px;"></div>
        <div class="blob" style="width:260px;height:260px;background:rgba(56,189,248,.07);bottom:-60px;left:-80px;"></div>

        {{-- Sheep & nature decorations --}}
        <span class="deco" style="top:5%;right:4%;font-size:2rem;opacity:.4;animation:sheepBounce 4s ease-in-out infinite;z-index:0;">🐑</span>
        <span class="deco" style="bottom:calc(var(--nav-pb) + 5px);left:5%;font-size:1.5rem;opacity:.4;animation:twinkle 3s ease-in-out infinite alternate;z-index:0;">🌿</span>
        <span class="deco" style="top:10%;left:3%;font-size:1.2rem;opacity:.35;animation:twinkle 2.5s ease-in-out infinite alternate;z-index:0;">🌸</span>
        <span class="deco" style="bottom:calc(var(--nav-pb) + 8px);right:4%;font-size:1.5rem;opacity:.35;animation:twinkle 3.5s .5s ease-in-out infinite alternate;z-index:0;">🌸</span>
    ...
    <section class="snap" id="s-events" data-section="2">
        <span class="deco" style="top:6%;right:4%;font-size:2rem;opacity:.18;animation:sheepBounce 4.5s ease-in-out infinite;z-index:0;">🐑</span>
        <span class="deco" style="top:12%;left:3%;font-size:1.5rem;opacity:.2;animation:twinkle 3s ease-in-out infinite alternate;z-index:0;">🌿</span>
    ...
    <section class="snap" id="s-gallery" data-section="3"
        ...
        <div class="gal-blob" style="width:280px;height:280px;background:rgba(22,163,74,.22);top:-50px;right:-40px;z-index:0;"></div>
        <div class="gal-blob" style="width:220px;height:220px;background:rgba(234,179,8,.15);bottom:80px;left:-40px;z-index:0;"></div>
        <div class="gal-blob" style="width:180px;height:180px;background:rgba(56,189,248,.12);top:40%;right:80px;z-index:0;"></div>
        ...
        <div class="gal-strip">
            ...
            @empty
                @for ($p = 0; $p < 6; $p++)
                    <div class="gal-card">
                        <div style="flex:1;background:linear-gradient(135deg,#0a1f10,#14532d);display:flex;align-items:center;justify-content:center;font-size:2.5rem;">🐑</div>
                        <div class="gal-cap">🌿</div>
                    </div>
                @endfor
            @endforelse
    ...
    <section class="snap" id="s-rsvp" data-section="4">
        <div style="position:absolute;border-radius:50%;filter:blur(70px);width:280px;height:280px;background:rgba(22,163,74,.06);top:-80px;right:-80px;pointer-events:none;z-index:0;"></div>
        <div style="position:absolute;border-radius:50%;filter:blur(70px);width:240px;height:240px;background:rgba(234,179,8,.05);bottom:-60px;left:-60px;pointer-events:none;z-index:0;"></div>
        ...
        <div class="rsvp-inner">
            ...
            <div class="rsvp-success" id="rsvpSuccess">
                ...
                <div class="success-text">Konfirmasi sudah kami terima.<br>Mohon doanya untuk <strong>{{ $invitation->profile->first_name }}</strong> 🤲</div>
            </div>
        </div>
    </section>
    ...
    <section class="snap" id="s-closing" data-section="5">
        ...
        <span style="position:absolute;top:7%;left:4%;font-size:clamp(2rem,6vw,3rem);opacity:.45;pointer-events:none;animation:sheepBounce 3.5s ease-in-out infinite;">🐑</span>
        <span style="position:absolute;top:9%;right:5%;font-size:clamp(1.5rem,4vw,2.2rem);opacity:.4;pointer-events:none;animation:twinkle 2.5s ease-in-out infinite alternate;">🌙</span>
        <span style="position:absolute;bottom:calc(var(--nav-pb) + 10px);left:8%;font-size:2rem;opacity:.4;pointer-events:none;animation:twinkle 3s ease-in-out infinite alternate;">🌿</span>
        <span style="position:absolute;bottom:calc(var(--nav-pb) + 15px);right:7%;font-size:2rem;opacity:.4;pointer-events:none;animation:twinkle 3.5s .5s ease-in-out infinite alternate;">⭐</span>
        ...
        <div class="cl-body">
            ...
            <p class="cl-parents">
                Putra/Putri dari {{ $invitation->profile->first_father }} &amp; {{ $invitation->profile->first_mother }}
            </p>
        </div>
    </section>
...
<nav id="bottom-nav" aria-label="Navigasi">
    ...
    <button class="n-btn"       data-target="s-rsvp"    onclick="navTo(this)"><span class="n-ico">🤲</span><span class="n-lbl">RSVP</span></button>
...
(function(){
    ...
    for(let i=0;i<65;i++){
        const s=document.createElement('div'); s.className='cv-dot';
        const sz=Math.random()*2.5+.8;
        const col=['rgba(253,230,138,','rgba(255,255,255,','rgba(134,239,172,'][Math.random()*3|0];
        s.style.cssText=`width:${sz}px;height:${sz}px;background:${col}1);left:${Math.random()*100}%;top:${Math.random()*100}%;position:absolute;animation:twinkle ${(Math.random()*2+1.5).toFixed(2)}s ${(Math.random()*2).toFixed(2)}s ease-in-out infinite alternate;`;
        bg.appendChild(s);
    }
})();
