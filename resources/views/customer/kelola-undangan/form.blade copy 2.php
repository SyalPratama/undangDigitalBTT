...
@section('content')
    ...
    <div class="flex h-[calc(100vh-4.5rem)] overflow-hidden font-sans border-t border-slate-200 -mx-6 -mt-6 bg-slate-100">
                ...
                <form id="auto-save-form"
                    ...
                    <div id="theme-builder-panel" class="hidden">
                        <div class="flex-1 overflow-y-auto p-6">
                            <form method="POST" action="{{ route('theme-builder.update', $invitation->id) }}"
                                id="auto-save-form" enctype="multipart/form-data">
                                @csrf @method('PUT')
        ...
        <div class="flex-1 flex flex-col relative overflow-hidden bg-slate-200/60 dot-pattern">
            ...
            {{-- Header Tools Iframe --}}
            <div
                class="absolute top-4 left-1/2 -translate-x-1/2 z-10 flex items-center bg-white/90 backdrop-blur-md p-1.5 rounded-xl shadow-sm border border-slate-200 gap-1">
                <button type="button" onclick="changeDevice('mobile')" id="btn-mobile"
                    class="device-btn px-4 py-1.5 rounded-lg bg-slate-800 text-white text-xs font-semibold transition-all">
                    <i class="fa-solid fa-mobile-screen mr-1"></i> Mobile
                </button>
                <button type="button" onclick="changeDevice('tablet')" id="btn-tablet"
                    class="device-btn px-4 py-1.5 rounded-lg bg-transparent text-slate-500 hover:text-slate-800 hover:bg-slate-100 text-xs font-semibold transition-all">
                ...
                <button onclick="refreshPreview()"
                    class="px-3 py-1.5 text-slate-500 hover:text-sky-500 hover:bg-sky-50 rounded-lg transition-all"
            ...
            <div id="preview-container-box"
                class="flex-1 overflow-y-auto flex items-center justify-center p-8 pt-20 custom-scrollbar relative">
                @if (isset($invitation) && $invitation->exists && $invitation->theme_id)
                    <div id="preview-wrapper"
                        class="relative transition-all duration-500 shadow-2xl bg-white border-[12px] border-slate-800 rounded-[2.5rem] overflow-hidden"
    ...
    <script>
        ...
        window.changeDevice = function(device) {
            ...
            if (device === 'mobile') {
                ...
                wrapper.className =
                    "relative transition-all duration-500 shadow-2xl bg-white border-[12px] border-slate-800 rounded-[2.5rem] overflow-hidden flex-shrink-0";
                ...
                wrapper.className =
                    "relative transition-all duration-500 shadow-2xl bg-white border-[16px] border-slate-800 rounded-[3rem] overflow-hidden flex-shrink-0";
