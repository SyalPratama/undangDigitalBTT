    ...
    <div class="space-y-6" x-data="{ openDetail: false, selectedData: {} }">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-xs p-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                ...
                <a href="{{ route('customer.kelola-undangan.create') }}"
                    ...
                    <tbody class="text-sm text-slate-600 divide-y divide-slate-50">
                        @forelse($invitations as $invitation)
                            <tr class="hover:bg-slate-50/40 transition-colors">
                                ...
                                {{-- Kolom 5: Status --}}
                                <td class="py-4 align-top">
                                    <form action="{{ route('customer.kelola-undangan.toggle-status', $invitation->id) }}"
                                        ...
                                        @if ($invitation->is_active)
                                            {{-- Jika true / 1 --}}
                                            <button type="submit"
                                                class="inline-flex items-center justify-center gap-2 px-3 h-8 text-emerald-600 bg-emerald-50 hover:bg-emerald-100/80 border border-emerald-200/30 rounded-xl transition-all cursor-pointer text-xs font-medium"
                                        ...
                                        @else
                                            {{-- Jika false / 0 --}}
                                            <button type="submit"
                                                class="inline-flex items-center justify-center gap-2 px-3 h-8 text-slate-400 bg-slate-50 hover:bg-slate-100/80 border border-slate-200/30 rounded-xl transition-all cursor-pointer text-xs font-medium"
                                                title="Aktifkan Undangan">
                                                <i class="fa-solid fa-toggle-off text-sm"></i>
                                                <span>Nonaktif</span>
                                            </button>
                                        @endif
                                    </form>
                                ...
                                <td class="py-4 text-right space-x-1 whitespace-nowrap align-top">
                                    <div class="mt-0.5">
                                        ...
                                        <a href="{{ route('customer.kelola-undangan.edit', $invitation->id) }}"
        ...
        function handleDelete(id, title) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.action = `/customer/kelola-undangan/${id}`;
                    form.submit();
                }
            });
        }
