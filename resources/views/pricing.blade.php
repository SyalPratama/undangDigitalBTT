...
@section('content')
    ...
    <div id="pricing" x-data class="mt-10 mb-10 max-w-6xl mx-auto px-4 text-center">
        ...
        @if (session('success'))
            <div
                class="max-w-2xl mx-auto mb-6 bg-emerald-50 text-emerald-700 px-4 py-3 rounded-xl border border-emerald-200 text-sm">
                {{ session('success') }}
            </div>
        @endif
        @if (session('error'))
        ...
        @if ($errors->any())
            <div
                class="max-w-2xl mx-auto mb-6 bg-red-50 text-red-700 px-4 py-3 rounded-xl border border-red-200 text-sm text-left">
        ...
        <div class="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8 items-start text-left max-w-5xl mx-auto">
            @foreach ($packages as $index => $package)
                ...
                <div
                    ...
                    <div>
                        <h3 class="text-xl font-bold text-slate-900 {{ $isPopular ? 'mt-2' : '' }}">{{ $package->name }}
                        </h3>
                        ...
                        <div class="mt-6 flex items-baseline text-slate-900">
                            ...
                            <span
                                class="text-4xl font-extrabold tracking-tight ml-1">{{ number_format($package->price, 0, ',', '.') }}</span>
                        </div>
                        ...
                        <ul role="list" class="mt-8 space-y-4 text-xs text-slate-600">
                            ...
                            @foreach ($featureItems as $feature)
                                <li class="flex items-start gap-2.5 {{ $feature['has'] ? '' : 'opacity-40' }}">
                                    ...
                                    @endif
                                    <span
                                        class="{{ $feature['has'] ? 'font-medium' : 'line-through text-slate-400' }}">{{ $feature['name'] }}</span>
                                </li>
                            @endforeach
                        </ul>
                    </div>
        ...
        <p class="mt-12 text-xs text-slate-500">
            Butuh kustom enterprise?
            <a href="https://wa.me/628152022225" target="_blank" class="text-purple-600 font-semibold hover:underline">
                Hubungi kami
            </a>
        </p>
    </div>
        ...
        <div @click.away="showModal = false"
            ...
            <div class="px-6 py-6 overflow-y-auto max-h-[80vh] text-left">
                ...
                <form action="/transactions" method="POST" enctype="multipart/form-data" id="paymentForm">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="relative group">
                            <input type="file" name="proof_of_payment" id="proof_of_payment"
                                ...
                                required>
                            <div
                                class="w-full bg-white border border-slate-300 group-hover:border-purple-400 rounded-xl py-3 px-4 flex items-center gap-3 transition">
                                <span
                                    class="bg-slate-50 text-slate-600 text-xs font-semibold px-3 py-1.5 rounded-lg border border-slate-200 shrink-0">Pilih
                                    File</span>
                        ...
                        <div x-show="previewUrl"
                            class="mt-3 relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50 flex justify-center p-2"
                            style="display: none;">
                    ...
                    <div class="mb-8">
                        <label for="notes" class="block text-sm font-semibold text-slate-700 mb-1.5">Catatan <span
                                class="text-slate-400 font-normal">(Opsional)</span></label>
                        <textarea name="notes" id="notes" rows="2"
                            class="w-full border border-slate-300 rounded-xl p-3 text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition"
                    ...
                    <div class="flex gap-3 pt-4 border-t border-slate-100">
                        <button type="button" @click="showModal = false"
                            class="flex-1 py-3 px-4 bg-white border border-slate-200 hover:border-slate-300 text-slate-700 font-semibold text-sm rounded-full hover:bg-slate-50 transition">
                            Batal
                        </button>
                        <button type="submit"
                            class="flex-1 py-3 px-4 bg-[#4c229a] hover:bg-[#3b1979] text-white font-semibold text-sm rounded-full transition shadow-md shadow-purple-900/10">
                            Kirim Pembayaran
                        </button>
                    </div>
                </form>
