...
<body class="text-slate-800 antialiased bg-gradient-to-br from-[#f4e6fa] via-[#e5f4fd] to-[#d6fbfb] min-h-screen flex overflow-hidden">
    ...
    <aside class="w-[260px] bg-white flex flex-col shrink-0 min-h-screen border-r border-slate-100 z-10">
        ...
        <div class="px-6 pb-2">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50 px-2.5 py-1 rounded-md border border-slate-200/40">Superadmin</span>
        </div>
        ...
        <nav class="flex-1 px-4 space-y-1 overflow-y-auto custom-scroll mt-2">
            ...
            <a href="{{ route('superadmin.user.index') }}"
                ...
                <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                Reseller & Customer
            </a>
        ...
        <div class="p-6 shrink-0">
            <a href="{{ route('superadmin.kelola-undangan.create') }}" class="w-full py-2.5 bg-white border border-slate-200 rounded-full text-[13px] font-semibold text-slate-700 hover:bg-slate-50 hover:text-slate-900 flex justify-center items-center gap-2 mb-4 transition-colors shadow-sm">
            ...
            <div class="border border-slate-200 rounded-[1.25rem] p-4 shadow-sm bg-white">
                <div class="text-[11px] text-slate-500 mb-2 truncate font-medium">{{ Auth::user()->email ?? 'superadmin@ngajak.com' }}</div>
