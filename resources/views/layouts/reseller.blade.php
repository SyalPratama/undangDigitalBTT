    ...
    <aside class="w-[260px] bg-white flex flex-col shrink-0 min-h-screen border-r border-slate-100 z-10">
        ...
        <nav class="flex-1 px-4 space-y-1 overflow-y-auto custom-scroll">
            ...
            <a href="{{ route('reseller.dashboard') }}"
                ...
                {{ request()->routeIs('reseller.dashboard') ? 'bg-[#6d28d9] text-white shadow-md shadow-purple-500/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2H6a2 2 0 01-2-2v-4zM14 16a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2v-4z"></path></svg>
                Dashboard
            </a>

            {{-- Templates --}}
            <a href="{{ route('reseller.templates.index') }}"
                class="flex items-center gap-3 px-4 py-3 text-[14px] font-medium rounded-full transition-all duration-200
                {{ request()->routeIs('reseller.templates.*') ? 'bg-[#6d28d9] text-white shadow-md shadow-purple-500/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
            ...
            <a href="{{ route('reseller.kelola-undangan.index') }}"
                class="flex items-center gap-3 px-4 py-3 text-[14px] font-medium rounded-full transition-all duration-200
                {{ request()->routeIs('reseller.kelola-undangan.*') ? 'bg-[#6d28d9] text-white shadow-md shadow-purple-500/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
            ...
            {{-- Paket Saya --}}
            <a href="{{ route('reseller.paket.index') }}"
                class="flex items-center gap-3 px-4 py-3 text-[14px] font-medium rounded-full transition-all duration-200
                {{ request()->routeIs('reseller.paket.*') ? 'bg-[#6d28d9] text-white shadow-md shadow-purple-500/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
        ...
        <div class="p-6 shrink-0">
            <a href="{{ route('reseller.kelola-undangan.index') }}" class="w-full py-2.5 bg-white border border-slate-200 rounded-full text-[13px] font-semibold text-slate-700 hover:bg-slate-50 hover:text-slate-900 flex justify-center items-center gap-2 mb-4 transition-colors shadow-sm">
            ...
            <div class="border border-slate-200 rounded-[1.25rem] p-4 shadow-sm bg-white">
                <div class="text-[11px] text-slate-500 mb-2 truncate font-medium">{{ Auth::user()->email ?? 'reseller@ngajak.com' }}</div>
