...
@section('content')
    ...
    <div id="modal-add"
        ...
        <div class="bg-white rounded-2xl max-w-md w-full shadow-lg p-6 transform transition-all">
            ...
            <form action="{{ route('reseller.user.store') }}" method="POST" class="space-y-4" id="form-add">
                @csrf
                <div>
    ...
    <script>
        ...
        function openEditModal(id, name, email, rolesJson) {
            document.getElementById('form-edit').action = `/reseller/kelola-user/${id}`;
        ...
        function handleDelete(id, name) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.action = `/reseller/kelola-user/${id}`;
                    form.submit();
                }
            });
        }
