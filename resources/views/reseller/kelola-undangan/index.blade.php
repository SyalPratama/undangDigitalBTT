                ...
                @forelse($invitations as $invitation)
                    <div class="py-5 flex flex-col md:flex-row md:items-center justify-between gap-4 group transition-colors">
                        <div>
                            ...
                            <p class="text-[12px] text-slate-500 mt-1">/invite/{{ $invitation->slug }} &middot; {{ rand(0,100) }} views</p>
                        </div>
                        
                        <div class="flex items-center gap-3 shrink-0">
                            {{-- Draft Badge --}}
                            <span class="inline-flex px-3 py-1.5 bg-slate-100/80 text-slate-500 rounded-full text-[11px] font-semibold tracking-wide">
                                {{ $invitation->is_active ? 'Active' : 'Draft' }}
                            </span>
                            
                            {{-- Tamu & Lokasi Button --}}
                            <button class="inline-flex items-center gap-1.5 px-4 py-1.5 bg-purple-100/50 text-[#6d28d9] rounded-full text-[11px] font-bold tracking-wide hover:bg-purple-100 transition-colors">
                            ...
                            {{-- Delete Button --}}
                            <form action="{{ route('reseller.kelola-undangan.destroy', $invitation->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus undangan ini?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2 text-rose-400 hover:text-rose-600 transition-colors" title="Delete">
