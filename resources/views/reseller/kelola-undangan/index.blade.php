...
@section('content')
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <a href="{{ route('reseller.kelola-undangan.create') }}" class="inline-flex items-center gap-2 bg-[#6d28d9] hover:bg-[#5b21b6] text-white px-6 py-2.5 rounded-full font-medium text-[14px] transition-all shadow-sm">
        ...
        <form method="GET" action="{{ route('reseller.kelola-undangan.index') }}" class="flex gap-3 mb-6 hidden">
                ...
                @forelse($invitations as $invitation)
                    <div class="py-5 flex flex-col md:flex-row md:items-center justify-between gap-4 group transition-colors">
                        ...
                        <div class="flex items-center gap-3 shrink-0">
                            ...
                            {{-- Share Link Button --}}
                            <button type="button" 
                                onclick="shareLink('{{ $invitation->id }}', '{{ $invitation->custom_domain ? route('invitation.subdomain', ['subdomain' => $invitation->custom_domain]) : route('invitation.show', $invitation->slug) }}', {{ $invitation->is_active ? 'true' : 'false' }}, '{{ route('reseller.kelola-undangan.toggle-status', $invitation->id) }}')" 
                            ...
                            @if(auth()->user()->hasFeature('has_rsvp'))
                                <a href="{{ route('reseller.kelola-undangan.guests', $invitation->id) }}" class="inline-flex items-center gap-1.5 px-4 py-1.5 bg-purple-100/50 text-[#6d28d9] rounded-full text-[11px] font-bold tracking-wide hover:bg-purple-100 transition-colors">
                            ...
                            @else
                                <button onclick="showLockedAlert('Tamu & Lokasi', '{{ route('reseller.paket.index') }}')" class="inline-flex items-center gap-1.5 px-4 py-1.5 bg-slate-100 text-slate-400 rounded-full text-[11px] font-bold tracking-wide cursor-pointer hover:bg-slate-200 transition-colors" title="Upgrade paket untuk fitur Tamu & Lokasi">
                                    <i class="fa-solid fa-lock text-[10px]"></i> Tamu & Lokasi
                                </button>
                            @endif
                            
                            {{-- Peta Button --}}
                            <a href="{{ route('reseller.kelola-undangan.map', $invitation->id) }}" class="inline-flex items-center gap-1.5 px-4 py-1.5 bg-purple-100/50 text-[#6d28d9] rounded-full text-[11px] font-bold tracking-wide hover:bg-purple-100 transition-colors">
                            ...
                            {{-- Edit Button --}}
                            <a href="{{ route('reseller.kelola-undangan.edit', $invitation->id) }}" class="p-2 text-slate-400 hover:text-slate-700 transition-colors ml-2" title="Edit">
                            ...
                            {{-- Delete Button --}}
                            <form action="{{ route('reseller.kelola-undangan.destroy', $invitation->id) }}" method="POST" class="inline-block" id="delete-form-{{ $invitation->id }}">
                                @csrf
                                @method('DELETE')
