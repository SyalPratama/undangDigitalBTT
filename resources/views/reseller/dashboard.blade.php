...
@section('content')
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <a href="{{ route('reseller.kelola-undangan.index') }}" class="inline-flex items-center gap-2 bg-[#6d28d9] hover:bg-[#5b21b6] text-white px-6 py-2.5 rounded-full font-medium text-[14px] transition-all shadow-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-white/60 backdrop-blur-md rounded-[2rem] p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] border border-white">
                <div class="flex justify-between items-start mb-8">
                    ...
                    <svg class="w-4 h-4 text-[#6d28d9]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                </div>
            ...
            <div class="bg-white/60 backdrop-blur-md rounded-[2rem] p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] border border-white">
                <div class="flex justify-between items-start mb-8">
                    ...
                    <svg class="w-4 h-4 text-[#6d28d9]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg>
                </div>
        ...
        <div class="bg-white/70 backdrop-blur-xl rounded-[2rem] p-8 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] border border-white mt-8">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('reseller.kelola-undangan.index') }}" class="text-[13px] font-medium text-slate-500 hover:text-slate-800">View all</a>
            </div>
            ...
            <div class="divide-y divide-slate-100/60">
                @forelse($recentInvitations as $invitation)
                    <div class="py-4 flex items-center justify-between group cursor-pointer hover:bg-white/50 px-4 -mx-4 rounded-2xl transition-colors">
                        <div>
                            <h4 class="text-[15px] font-semibold text-slate-800">{{ $invitation->title ?? 'Untitled Aurora Glass' }}</h4>
                            <p class="text-[12px] text-slate-500 mt-0.5">/{{ $invitation->slug ?? 'aurora-glass-vumfe' }} &middot; {{ rand(0,100) }} views</p>
                        </div>
                        <div>
                            <span class="inline-flex px-3 py-1 bg-slate-100/80 text-slate-500 rounded-full text-[11px] font-semibold tracking-wide">
                                Draft
                            </span>
                        </div>
                    </div>
                @empty
                    <div class="py-4 flex items-center justify-between group cursor-pointer hover:bg-white/50 px-4 -mx-4 rounded-2xl transition-colors">
                        <div>
                            <h4 class="text-[15px] font-semibold text-slate-800">Untitled Aurora Glass</h4>
                            <p class="text-[12px] text-slate-500 mt-0.5">/aurora-glass-vumfe &middot; 0 views</p>
                        </div>
                        <div>
                            <span class="inline-flex px-3 py-1 bg-slate-100/80 text-slate-500 rounded-full text-[11px] font-semibold tracking-wide">
                                Draft
